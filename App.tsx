...
function App() {
  ...
  return (
    <div className="h-screen w-full bg-[#141218] text-[#E6E1E5] flex flex-col md:flex-row font-sans overflow-hidden">
      ...
      <nav className="hidden md:flex flex-col w-24 bg-[#1D1B20] h-full items-center py-8 z-20 border-r border-[#49454F]">
      ...
      {/* Main Content Area */}
      <main className="flex-grow relative flex flex-col h-full overflow-hidden z-10">
        
        {/* Top App Bar (Mobile & Desktop) */}
        <header className="h-16 md:h-20 flex items-center justify-between px-4 md:px-8 bg-[#141218]/80 backdrop-blur-md sticky top-0 z-30 border-b border-[#49454F]/30 md:border-none transition-all">
           {/* Logo Area: Hidden on mobile when Tree view is active to give space to Selector */}
           <div className={`flex items-center gap-3 ${currentView === 'tree' ? 'hidden md:flex' : 'flex'}`}>
              <div className="md:hidden w-8 h-8 rounded-lg bg-amber-500 flex items-center justify-center text-[#141218] font-bold font-serif">ע</div>
              <h1 className="text-xl md:text-2xl font-serif text-[#E6E1E5]">
                Etz Chaim 
                {currentView !== 'tree' && <span className="text-[#CAC4D0] text-lg font-sans ml-2 opacity-70">| {views.find(v => v.id === currentView)?.label}</span>}
              </h1>
           </div>
           ...
           {currentView === 'tree' && (
             <div className="flex bg-[#1D1B20] rounded-full p-1 border border-[#49454F] shadow-inner items-center ml-auto md:ml-0">
                {worlds.map((w) => {
                  ...
                  return (
                    <button
                      ...
                      className={`
                        relative px-2 md:px-5 py-1.5 md:py-1.5 text-[10px] md:text-xs font-medium rounded-full transition-all duration-300
                        ${isActive 
        ...
        <div className="flex-grow overflow-y-auto overflow-x-hidden p-4 md:p-6 flex justify-center pb-24 md:pb-6">
          <div className="w-full max-w-lg md:max-w-4xl animate-fade-in">
            {currentView === 'tree' && (
              <TreeVisualizer 
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#1D1B20] border-t border-[#49454F] z-50 pb-safe">
        <div className="flex justify-around items-center h-20">
          {views.map((v) => (
             <button
               key={v.id}
               onClick={() => setCurrentView(v.id)}
               className={`flex flex-col items-center gap-1 w-full py-2 group ${
             ...
             >
               <div className={`px-5 py-1 rounded-full transition-all ${
                 ...
                 <span className={`material-symbols-rounded text-2xl ${currentView === v.id ? 'filled' : ''}`}>{v.icon}</span>
               </div>
               <span className={`text-xs font-medium ${currentView === v.id ? 'font-bold' : ''}`}>{v.label}</span>
             </button>
          ))}
        </div>
      </nav>
      ...
      {(activeSefira || activePath) && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm animate-fade-in" onClick={closeModal}>
          <div 
            className="bg-[#1D1B20] w-full h-full md:h-auto md:w-full md:max-w-lg md:max-h-[85vh] md:rounded-[28px] shadow-2xl relative flex flex-col overflow-hidden animate-slide-up md:animate-zoom-in" 
            onClick={e => e.stopPropagation()}
          >
            {/* Close Button */}
            <button onClick={closeModal} className="absolute top-4 right-4 text-[#CAC4D0] hover:text-[#E6E1E5] z-20 bg-black/20 rounded-full p-2 backdrop-blur-md">
            ...
            {activeSefira && (
              <>
                ...
                <div className={`p-8 pb-6 relative overflow-hidden shrink-0`}>
                   ...
                   <div className="relative z-10 text-center mt-6 md:mt-0">
                      <div className="flex justify-center mb-3">
                         <span className="text-[11px] uppercase tracking-widest text-[#E6E1E5] border border-[#938F99] px-3 py-1 rounded-full bg-[#141218]/50 backdrop-blur-sm">{world}</span>
                      </div>
                      <div className="text-6xl hebrew mb-2 font-serif opacity-90">{activeSefira.hebrewName}</div>
                      <h2 className="text-4xl font-bold font-serif text-[#E6E1E5]">{activeSefira.name}</h2>
                ...
                <div className="p-6 md:p-8 space-y-6 overflow-y-auto">
                   {/* Essence */}
                   <section>
                     <p className="text-[#E6E1E5] leading-relaxed text-base italic border-l-4 border-[#FFDDA2] pl-4 py-1">
                   ...
                   <div className="grid grid-cols-2 gap-4 text-sm">
                      <div className="bg-[#2B2930] p-4 rounded-2xl">
                        <span className="block text-xs text-[#CAC4D0] uppercase font-bold mb-1">Virtud</span>
                        <span className="text-emerald-300 font-medium text-base">{activeSefira.virtue}</span>
                      </div>
                      <div className="bg-[#2B2930] p-4 rounded-2xl">
                        <span className="block text-xs text-[#CAC4D0] uppercase font-bold mb-1">Vicio</span>
                        <span className="text-rose-300 font-medium text-base">{activeSefira.vice}</span>
                      </div>
                      <div className="bg-[#2B2930] p-4 rounded-2xl">
                        <span className="block text-xs text-[#CAC4D0] uppercase font-bold mb-1">Planeta</span>
                        <span className="text-indigo-300 font-medium text-base">{activeSefira.planet}</span>
                      </div>
                      <div className="bg-[#2B2930] p-4 rounded-2xl">
                        <span className="block text-xs text-[#CAC4D0] uppercase font-bold mb-1">Pilar</span>
                        <span className="text-amber-300 font-medium text-base">{activeSefira.pillar}</span>
                      </div>
                      <div className="bg-[#2B2930] p-4 rounded-2xl col-span-2 md:col-span-2">
                        <span className="block text-xs text-[#CAC4D0] uppercase font-bold mb-1">Arquetipo</span>
                        <span className="text-purple-200 font-medium text-base">{activeSefira.archetype}</span>
                      </div>
                   </div>

                   {/* Practice Section */}
                   <section className="bg-[#2B2930] rounded-[24px] p-6">
                     <h3 className="text-lg font-serif text-[#FFDDA2] mb-4 flex items-center gap-2">
                        <span className="material-symbols-rounded text-[#FFDDA2]">self_improvement</span>
                        <span>Práctica</span>
                     </h3>
                     <div className="mb-5">
                        <p className="text-xs text-[#CAC4D0] uppercase font-bold mb-2">Pregunta Clave</p>
                        <p className="text-[#E6E1E5] font-medium text-lg leading-snug">¿{activeSefira.question}</p>
                     </div>
                     <div className="mb-5">
                        <p className="text-xs text-[#CAC4D0] uppercase font-bold mb-2">Ejercicios</p>
                        <ul className="list-disc list-inside text-[#E6E1E5] text-sm space-y-2 marker:text-[#FFDDA2]">
                     ...
                     <div className="bg-[#141218] p-4 rounded-xl text-center border border-[#49454F]">
                        ...
                        <p className="text-[#FFDDA2] italic font-serif text-lg">"{activeSefira.affirmation}"</p>
                     </div>
                   </section>
                </div>
              </>
            ...
            {activePath && (
              <>
                <div className="p-8 pb-0 text-center border-b border-[#49454F]/50 bg-[#2B2930] pt-12 md:pt-8">
                ...
                <div className="p-6 md:p-8 space-y-5 overflow-y-auto">
                     <div className="flex justify-between text-sm bg-[#2B2930] p-4 rounded-2xl">
                     ...
                     <div className="bg-[#1D1B20] p-2">
                        <h3 className="text-sm font-bold text-[#CAC4D0] uppercase mb-2">Significado</h3>
                        <p className="text-[#E6E1E5] text-base leading-relaxed">{activePath.description}</p>
                     </div>

                     <div className="bg-[#4F378B]/20 border border-[#4F378B] p-5 rounded-2xl">
                        <h3 className="text-sm font-bold text-[#D0BCFF] uppercase mb-2 flex items-center gap-2">
                          <span className="material-symbols-rounded text-sm">psychology</span> Integración
                        </h3>
                        <p className="text-[#E6E1E5] text-sm mb-4">{activePath.practice}</p>
