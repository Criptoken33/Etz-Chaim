...
const DailyPractice: React.FC = () => {
  ...
  return (
    <div className="w-full bg-[#1D1B20] rounded-[24px] overflow-hidden shadow-sm border border-[#49454F] md:max-w-md mx-auto">
      {/* Hero Image / Color bar */}
      <div className={`h-24 w-full relative ${dailySefira.colors.Yetzirah.replace('text-', 'bg-')} opacity-80`}>
      ...
      <div className="p-6 pt-2">
        <div className="flex justify-between items-end mb-6">
            <div>
              <div className="text-5xl hebrew text-[#E6E1E5] opacity-20 absolute top-4 right-4 pointer-events-none select-none">{dailySefira.hebrewName}</div>
              <h2 className="text-3xl font-serif text-[#E6E1E5]">{dailySefira.name}</h2>
              <p className="text-[#CAC4D0] text-sm">{dailySefira.meaning}</p>
            </div>
        </div>

        <div className="space-y-4">
            <div className="bg-[#2B2930] p-5 rounded-2xl">
                ...
                <p className="text-[#E6E1E5] text-lg font-medium leading-relaxed">{dailySefira.question}</p>
            </div>
            ...
            <div className="bg-[#2B2930] p-5 rounded-2xl">
                 ...
                 {loading ? <p className="text-[#938F99] animate-pulse">Sintonizando...</p> : <p className="text-[#E6E1E5] text-sm italic">"{guidance}"</p>}
            </div>

            <div className="mt-6">
                <h4 className="text-sm font-bold text-[#CAC4D0] mb-3 uppercase tracking-wider">Práctica Sugerida</h4>
                <ul className="space-y-3">
                    {dailySefira.practices.map((p, i) => (
                      <li key={i} className="flex items-start gap-3 text-sm text-[#E6E1E5]">
                        <span className="material-symbols-rounded text-[#FFDDA2] text-lg">check_circle</span>
      ...
      <div className="p-6 pt-0">
        <button className="w-full py-3.5 bg-[#FFDDA2] hover:bg-[#FFDDA2]/90 text-[#251A00] rounded-full text-sm font-bold tracking-wide transition-colors flex items-center justify-center gap-2">
